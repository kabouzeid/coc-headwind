...
const formatterPriority = config.get('headwind.formatterPriority', 0);
function activate(context) {
    function getSortTailwindClassesEdits(document) {
        const edits = [];
        ...
        while ((classWrapper = classWrapperRegex.exec(editorText)) !== null) {
            ...
            const range = coc_nvim_1.Range.create(document.positionAt(startPosition), document.positionAt(endPosition));
            const options = {
                shouldRemoveDuplicates,
                shouldPrependCustomClasses,
                customTailwindPrefix,
            };
            const newText = utils_1.sortClassString(valueMatch, Array.isArray(sortOrder) ? sortOrder : [], options);
            ...
            if (document.getText(range) != newText) {
                edits.push(coc_nvim_1.TextEdit.replace(range, newText));
            }
        }
        coc_nvim_1.window.showInformationMessage('Formatted by Headwind');
        return edits;
    }
    ...
    //       const promise = new Promise<TextEdit[] | undefined>((resolve) => {
    //         resolve(getSortTailwindClassesEdits(event.document));
    //       });
    ...
    //   );
    // }
    coc_nvim_1.languages.registerDocumentFormatProvider([
        { language: 'html' },
        { language: 'javascript' },
        { language: 'typescript' },
    ...
    coc_nvim_1.window.showInformationMessage(`registered ${formatterPriority}`);
}
